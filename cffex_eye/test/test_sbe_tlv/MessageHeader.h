...
public:
    ...
    {
        return SBE_BIG_ENDIAN_ENCODE_16(*((std::uint16_t *)(m_buffer + m_offset + 4)));
    }

    MessageHeader &schemaId(const std::uint16_t value)
    {
        *((std::uint16_t *)(m_buffer + m_offset + 4)) = SBE_BIG_ENDIAN_ENCODE_16(value);
        return *this;
    }
    ...
    {
        return SBE_BIG_ENDIAN_ENCODE_16(*((std::uint16_t *)(m_buffer + m_offset + 6)));
    }

    MessageHeader &version(const std::uint16_t value)
    {
        *((std::uint16_t *)(m_buffer + m_offset + 6)) = SBE_BIG_ENDIAN_ENCODE_16(value);
        return *this;
    }
};
}
